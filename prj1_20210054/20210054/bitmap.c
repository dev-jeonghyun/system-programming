...
void printBitmap(struct bitmap** bitmapArr, const char* name) {
    int index = atoi(name + 2);
    struct bitmap* bm = bitmapArr[index];
    if (bm == NULL) return;

    for (size_t pos = 0; pos < bitmap_size(bm); pos++) {
        putchar(bitmap_test(bm, pos) ? '1' : '0');
    }
    putchar('\n');
}
struct bitmap* bitmap_expand(struct bitmap* old_bm, size_t extra_bits) {
    size_t original_size = bitmap_size(old_bm);
    struct bitmap* extended = bitmap_create(original_size + extra_bits);

    for (size_t k = 0; k < original_size; ++k) {
        bitmap_set(extended, k, bitmap_test(old_bm, k));
    }

    bitmap_destroy(old_bm);
    return extended;
}
